...
func main() {
    ...
    passwordDB := flag.String("db-password", "admin", "Пароль пользователя БД")
    ...
    connectURI := fmt.Sprintf(
        "mongodb://%s:%s@%s/%s",
        *userDB, *passwordDB, *mongoOnAddr, *defaultDB)
    fmt.Println(connectURI)

    err = m.Connect(ctx, connectURI)
    if err != nil {
    ...
    app := &App{
        C: &Config{
            Addr: *runOnAddr, Secret: *secretWord,
            TTL: ttl{
